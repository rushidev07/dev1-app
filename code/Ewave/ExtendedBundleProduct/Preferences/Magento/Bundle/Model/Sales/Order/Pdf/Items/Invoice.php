...
namespace Ewave\ExtendedBundleProduct\Preferences\Magento\Bundle\Model\Sales\Order\Pdf\Items;
 ...
 * Class Invoice
 *
 * @package Ewave\ExtendedBundleProduct\Preferences\Magento\Bundle\Model\Sales\Order\Pdf\Items
 */
class Invoice extends AbstractItems
{
    ...
    public function draw()
    {
        ...
        foreach ($items as $childItem) {
            ...
            if ($childItem->getOrderItem()->getData('product_type') == Configurable::TYPE_CODE) {
                if ($superAttributes = $childItem->getOrderItem()->getProductOptionByCode('attributes_info')) {
                    foreach ($superAttributes as $superAttribute) {
                        $text = [];
                        $text[] = $superAttribute['label'];
                        $text[] = $superAttribute['value'];
                        if (count($text) == 2) {
                            $drawItems[$optionId]['lines'][] = [
                                [
                                    'text' => implode(': ', $text),
                                    'feed' => 40,
                                    'align' => 'left'
