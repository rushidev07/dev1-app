...
namespace Ahy\ThemeCustomization\Model\Klevu\Search\Product;
...
use Webkul\Marketplace\Helper\Data as Webkul_Marketplace_Helper;
use Magento\Store\Api\Data\StoreInterface;
use Magento\Store\Model\StoreManagerInterface;
use Ahy\ThemeCustomization\Helper\Data as Ahy_ThemeCustomization_Helper;

class LoadAttribute extends \Klevu\Search\Model\Product\LoadAttribute
{
    /**
    ...
    /**
     * @var Webkul_Marketplace_Helper
     */
    private $_webkulMarketPlaceHelper;
    /**
     * @var Ahy_ThemeCustomization_Helper
     */
    private $_ahyHelper;
    ...
    public function __construct(
        ...
        Webkul_Marketplace_Helper $webkulMarketPlaceHelper,
        Ahy_ThemeCustomization_Helper $themeCustomizationHelper
    ) {
        ...
        $this->_webkulMarketPlaceHelper = $webkulMarketPlaceHelper;
        $this->_ahyHelper = $themeCustomizationHelper;
    ...
    public function addProductSyncData(&$products)
    {
        ...
        foreach ($products as $index => &$product) {
            ...
            try {
                ...
                if($item->getPrice() >= 99999) {
                    ...
                    $this->_searchHelperData->log(
                        LoggerConstants::ZEND_LOG_WARN,
                        sprintf("this is a base product product ID %d", $product['product_id'])
                    );
                    unset($products[$index]);
                    $rp++;
                    continue;
                ...
                if ($productType == 'configurable') {
                    ...
                    foreach ($usedProducts as $simpleProduct) {
                        ...
                        $stockItem = $stockRegistry->getStockItem($simpleProduct->getId());
                    ...
                    if((int)$product['parent_id'] > 0){
                        $parentItem = $this->productRepository->getById(
                            $product['parent_id'],
                            false,
                            $store->getId()
                        );
                        $productTypeInstance = $parentItem->getTypeInstance();
                        $usedProducts = $productTypeInstance->getUsedProducts($parentItem);
                        $totalStockQty = 0;
                        foreach ($usedProducts as $simpleProduct) {
                            $stockRegistry = ObjectManager::getInstance()->get('\Magento\CatalogInventory\Api\StockRegistryInterface');
                            $stockItem = $stockRegistry->getStockItem($simpleProduct->getId());
                ...
                $stockJson = json_encode(array('stockQty' => $stockQty));

                // swatches info

                // Setting default swatch config for pure simple products
                $swatchConfigJson = '{}';
                $swatchConfig = [];
                if ($productType == 'configurable') {

                    $swatchAttributes = $item->getTypeInstance()->getConfigurableAttributesAsArray($item);

                    foreach ($swatchAttributes as $attribute) {
                        $options = array();
                        ...
                        $swatchConfig[$attribute->getProductAttribute()->getAttributeCode()] = array(
                            'attribute_id' => $attribute['attribute_id'],
                            'attribute_code' => $attribute->getProductAttribute()->getAttributeCode(),
                            'options' => $options,
                        );
                        $swatchConfigJson = json_encode($swatchConfig);
                    }
                } else {
                    //Check if the simple product belongs to a configurable product
                    if((int)$product['parent_id'] > 0){
                        ...
                        $swatchAttributes = $parentItem->getTypeInstance()->getConfigurableAttributes($parentItem);
                ...
                // Seller Info
                $_helper = $this->_webkulMarketPlaceHelper;
                $sellerId = 0;
                $freeShippingFromSeller = false;
                if ($productType == 'configurable') {
                    $seller = $_helper->getSellerProductDataByProductId($item->getId());
                } else {
                    //Check if the simple product belongs to a configurable product
                    if((int)$product['parent_id'] > 0){
                        // This simple product belongs to a configurable product
                        $parentItem = $this->productRepository->getById(
                            $product['parent_id'],
                            false,
                            $store->getId()
                        );
                        $seller = $_helper->getSellerProductDataByProductId($parentItem->getId());
                    } else {
                        // Pure Simple Product
                        $seller = $_helper->getSellerProductDataByProductId($item->getId());
                    }
                ...
                if(isset($seller->getData()[0]) && $seller->getData()[0]){
                    ...
                    $sellerJson = json_encode($sellerJsonData);
                }else{
                    $sellerJson = '{"shopUrl":"","shopTitle":"", "sellerId":""}';
                }
                $shippingText = $freeShippingFromSeller ? " \" FREE shipping\" " : " \" \" ";
                //!---------------------------test with logs----------------------
                // $this->_searchHelperData->log(
                //     LoggerConstants::ZEND_LOG_CRIT,
                //     sprintf('Seller Json............: %s', $sellerJson)
                // );die;

                $productTypeData = [
                    'productType' => $this->_productData->getProductType($parent, $item)
                ];
                $productTypeJson = json_encode($productTypeData);

                $productStatus = [
                    'productStatus' => $item->getNoLongerAvailable()
                ];
                $productStatusJson = json_encode($productStatus);

                // $this->_searchHelperData->log(
                //     LoggerConstants::ZEND_LOG_CRIT,
                //     sprintf('product status............: %s', $item->getNoLongerAvailable())
                // );die;
                ...
                $product['additionalDataToReturn'] = '{
                    "seller_data":'.$sellerJson.',
                    "swatches":'.$swatchConfigJson.',
                    "shipping_charge":'.$shippingText.',
                    "stock_qty":'.$stockJson.',
                    "product_types":'.$productTypeJson.',
                    "product_status":'.$productStatusJson.'
                }';
        ...
        if (count($rejectedProducts) > 0) {
            ...
            $magentoProductActions = ObjectManager::getInstance()->get(\Klevu\Search\Model\Product\MagentoProductActionsInterface::class);
