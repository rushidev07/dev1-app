            ...
            foreach ($fetchsale as $res) {
                ...
                */
                if ($this->_marketplaceHelper->getlowStockNotification()) {
                    // $stockState = $this->_objectManager->get(\Magento\InventorySalesAdminUi\Model\GetSalableQuantityDataBySku::class);
                    ...
                    // }
                    // $this->_marketplaceHelper->logDataInLogger( "Observer_SalesOrderPlaceAfterObserver productSalesCalculation line 403 : " . $sku . ' qty: ' . $stockItemQty . ' product Id : ' . $product->getId() . ' Realtime Qty: ' . $realTimeQty);
        ...
        */
        ...
        foreach ($order->getAllItems() as $item) {
            ...
            if ($sellerId && $sellerId != '') {
                ...
                if ($totalamount != 0) {
                    $totalAfterDiscount = $totalamount - $baseDiscountAmount;
                    ($totalAfterDiscount != 0) ? $collectionsave->setTotalAmount($totalamount) : $collectionsave->setTotalAmount($price);
                    $commissionRate = ($totalAfterDiscount != 0) ? (($commission * 100) / $totalAfterDiscount) : $percent;
                } else {
