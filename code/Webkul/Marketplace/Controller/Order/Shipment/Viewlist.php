...
/**
 * Webkul Marketplace Order Shipment Viewlist Controller.
 */
    ...
    protected function _initOrder()
    {
        ...
        try {
            ...
            $this->helper->logDataInLogger(
                "Controller_Order_Shipment_Viewlist _initOrder : ".$e->getMessage()
            );
            $this->_actionFlag->set('', self::FLAG_NO_DISPATCH, true);
        ...
        if ($isPartner == 1) {
            if ($order = $this->_initOrder()) {
                ...
                if ($helper->getIsSeparatePanel()) {
                    $resultPage->addHandle('marketplace_layout2_order_shipment_viewlist');
                }
                $resultPage->getConfig()->getTitle()->set(
                    __('Shipment - Order #%1', $order->getRealOrderId())
                );
