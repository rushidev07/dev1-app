 ...
 * @api
 */
class Packaging extends \Magento\Framework\View\Element\Template
{
    /**
     * Source size model
     *
    ...
    public function __construct(
        ...
        \Webkul\MarketplaceBaseShipping\Helper\Data $currentHelper,
        MpHelper $mpHelper,
        array $data = []
    ) {
    ...
    public function getCarrierObj()
    {
        $carrierCode = $this->getShipment()->getOrder()->getShippingMethod(true);
        return $this->_carrierFactory->createIfActive($carrierCode['carrier_code']);
    }
    ...
    public function getConfigDataJson()
    {
        ...
        if ($shipmentId) {
            ...
            $createLabelUrl = $this->getUrl('baseshipping/order_shipment/createLabel', $urlParams);
            $itemsGridUrl = $this->getUrl('baseshipping/order_shipment/getShippingItemsGrid', $urlParams);
            ...
            if ($orderId) {
                ...
                $createLabelUrl = $this->getUrl('baseshipping/order_shipment/save', $urlParams);
                $itemsGridUrl = $this->getUrl('baseshipping/order_shipment/getShippingItemsGrid', $urlParams);
    ...
    public function getContainers()
    {
        ...
        $carrier = $this->_carrierFactory->create($this->currentHelper->getCarrierCode($order));
        $customerId = $this->_customerSession->getCustomerId();
        $shipperAddress = $this->dataRepository->getBySellerId($customerId, $storeId);
        $countryShipper = $shipperAddress->getCountryId();
        if ($carrier) {
            $params = new \Magento\Framework\DataObject(
                [
                    'method' => $this->currentHelper->getShippingMethod($order)->getMethod(),
    ...
    public function displayCustomsValue()
    {
        ...
        $shipperAddressCountryCode = $shipperAddress->getCountryId();
    ...
    /**
     * Get marketplaceBaseshipping helper
     *
     ...
     * @return int
     */
    public function getGirthEnabled()
    {
        return $this->isDisplayGirthValue() && $this->isGirthAllowed() ? 1 : 0;
    }
}
