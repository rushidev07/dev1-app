...
class SyncLockManager
{
    ...
    private function ensureLockDir(): void
    {
        ...
        if (!is_dir($dir)) {
            // 0775 (not 0755) — matches every other var/ subdir (cache, log, page_cache).
            // The admin controller runs as the web server user (e.g. nginx), which is only
            // a *group* member of the file owner, not the owner itself, so the group-write
            // bit is required for it to create the lock file. Without it, acquire() fails
            // with a plain permission error that looks identical to "already locked."
            mkdir($dir, 0775, true);
            return;
        }

        // Self-heal: if the directory already exists with the wrong (group-non-writable)
        // mode from before this fix, correct it so acquire() doesn't keep failing silently.
        if ((fileperms($dir) & 0777) !== 0775) {
            @chmod($dir, 0775);
        }
    }
