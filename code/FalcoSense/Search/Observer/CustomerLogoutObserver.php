...
/**
 * Fires on: customer_logout (frontend scope)
 * Event data: customer — Magento\Customer\Model\Customer
 */
class CustomerLogoutObserver implements ObserverInterface
{
    ...
    public function execute(Observer $observer): void
    {
        ...
        try {
            ...
            $this->eventService->trackLogout(
                customerId: (int) $customer->getId(),
                storeId:    $storeId
            );
        } catch (\Throwable $e) {
            $this->logger->error('[SmartSearch][Events] CustomerLogout observer error: ' . $e->getMessage());
        }
    }
}
