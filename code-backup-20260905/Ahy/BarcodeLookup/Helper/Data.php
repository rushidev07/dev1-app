    ...
    public function createJsonFileFromCsvFile(): array
    {
        try {
            ...
            foreach ($upcs as $index => $upc) {
                ...
                // Start: To handle the case where barcode API itself returns an incorrect UPC with a leading zero. E.g. https://api.barcodelookup.com/v3/products?barcode=65925026856,65925415117,65925035162,70382012830,70382013202,70382013219,70382013226,91887221619,91887221626,91887221633&formatted=y&key=3sankds08d2w6cqqv6q0emsic73qe5

                $upcArray = explode(',', $upc);
                $decodedResponse = json_decode($apiResponse['response'], true);
                if (isset($decodedResponse['products']) && is_array($decodedResponse['products'])) {
                    foreach ($decodedResponse['products'] as $index => &$product) {
                        if (isset($upcArray[$index])) {
                            $product['barcode_number'] = $upcArray[$index];
                        }
                    }
                }
