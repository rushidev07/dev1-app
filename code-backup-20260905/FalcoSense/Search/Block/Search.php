...
class Search extends Template
{
    ...
    public function getCustomerGeoState(): string
    {
        try {
            ...
            // Never let address lookup break the search page
        }
     ...
     * Address priority: default billing → default shipping → first address.
     */
    public function getCustomerGeoCountry(): string
    {
        try {
            if (!$this->customerSession->isLoggedIn()) {
                return '';
            ...
            foreach ($addressIds as $addressId) {
                ...
                if (trim((string) $address->getCountryId()) === '') {
                    continue;
                }

                try {
                    $countryName = trim((string) $address->getCountryModel()->getName());
                } catch (\Throwable) {
                    $countryName = '';
                }
