...
class Shipment extends \Magento\Sales\Model\Order\Pdf\Shipment
{
     ...
     *
     * @param \Zend_Pdf_Page $sellerPdfPage
     * @param \Magento\Sales\Model\Order $sellerOrder
     * @param \Magento\Sales\Model\Order\Shipment $sellerShipment
     * @param float $top
     */
    protected function doInsertOrderExecution($sellerPdfPage, $sellerOrder, $sellerShipment, $top)
    {
        ...
        if ($sellerOrder->getIsVirtual()) {
            ...
            if (count($tracks)) {
                ...
                foreach ($tracks as $track) {
                    $maxTitleLen = 45;
                    $titleLen = $track->getTitle() ?: '';
                    $endOfTitle = strlen($titleLen) > $maxTitleLen ? '...' : '';
                    $truncatedTitle = substr($titleLen, 0, $maxTitleLen) . $endOfTitle;
                    $sellerPdfPage->drawText($truncatedTitle, 292, $yShipments, 'UTF-8');
                    $sellerPdfPage->drawText($track->getNumber(), 410, $yShipments, 'UTF-8');
                    $yShipments -= $topMargin - 5;
                }
