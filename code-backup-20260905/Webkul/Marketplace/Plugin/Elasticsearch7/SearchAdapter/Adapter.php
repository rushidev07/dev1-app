...
class Adapter
{
    ...
    public function aroundQuery(
        ...
        } elseif ($this->helper->allowSellerFilter()) {
            $requestVar = $this->helper->getRequestVar();
            $param = $this->request->getParam($requestVar) ? $this->request->getParam($requestVar) : '';
            $filter = trim($param);
            if ($filter != "") {
                $sellerProductCollection = $this->collection;
                ...
                if ($filter != $this->helper::MARKETPLACE_ADMIN_URL) {
                ...
                $connection = $this->connection->query($query)->fetchAll();
                $productIds = array_column($connection, 'mageproduct_id');
                $client = $this->connectionManager->getConnection();
                $aggregationBuilder = $this->aggregationBuilder;
                $updatedQuery = $this->mapper->buildQuery($request);
                if ($filter == $this->helper::MARKETPLACE_ADMIN_URL) {
                   
                    $updatedQuery['body']['query']['bool']['mustNot'] = ['ids' => ['values' => $productIds]];
