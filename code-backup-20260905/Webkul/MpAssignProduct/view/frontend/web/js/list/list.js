            ...
            _create: function () {
                ...
                $(document).ready(function () {
                    $(".products ol.product-items > li.product-item").on('click', '.action.tocart', function (e) {
                        ...
                        if (assignInfo[productLink]["showMinimumPrice"] == 1) {
                            var assignProductID = assignInfo[productLink]["assignProductsID"];                            
                            var lastString = ajaxUrl.substring(ajaxUrl.length -1);
                            if (lastString === '/') {
                                ajaxUrl = ajaxUrl.substring(0,ajaxUrl.length - 1);
                                var mainProdId = ajaxUrl.split('/').pop();
                                ajaxUrl = ajaxUrl.substring(0, ajaxUrl.length - mainProdId.length)+assignProductID+'/';
                            } else {
                                var mainProdId = ajaxUrl.split('/').pop();
                                ajaxUrl = ajaxUrl.substring(0, ajaxUrl.length - mainProdId.length)+assignProductID+'/';
                            }                            
